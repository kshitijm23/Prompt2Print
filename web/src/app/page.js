...
const PACK_LABELS = { starter: "20 worksheets", classroom: "100 worksheets" };
...
function HomeInner() {
  ...
  useEffect(() => {
    if (searchParams.get("purchase") === "success") {
      ...
      setToastMessage(`🎉 Success — ${PACK_LABELS[pack] || "worksheets"} added to your account`);
      setShowToast(true);
  ...
  async function markTourSeen(uid) {
    ...
    } catch (err) { console.error("markTourSeen failed:", err); }
  }

  function launchTour(uid) {
    if (tourRef.current) { try { tourRef.current.cancel(); } catch {} }
    tourRef.current = startTour({
      onEnd: () => { tourRef.current = null; markTourSeen(uid); },
    });
  }
  ...
  useEffect(() => {
    (async () => {
      ...
      if (profile) {
        ...
        if (!profile.has_seen_tour) setTimeout(() => launchTour(userData.user.id), 400);
      }
    })();
    function onVisible() {
      if (document.visibilityState === "visible") {
        supabase.auth.getUser().then(({ data: userData }) => {
          if (!userData.user) return;
          supabase.from("user_profiles").select("credits_remaining").eq("user_id", userData.user.id).single()
            .then(({ data: profile }) => { if (profile) setCredits(profile.credits_remaining); });
        });
      }
    }
  ...
  async function generateWorksheet() {
    ...
    if (referenceFile) {
      ...
      sessionStorage.setItem("p2p-ref-b64", btoa(binary));
  ...
  return (
    <main className="relative min-h-screen bg-[color:#FAFAF6] overflow-hidden">
      ...
      {showToast && (
        <div className="fixed top-4 left-3 right-3 sm:top-6 sm:left-1/2 sm:right-auto sm:-translate-x-1/2 z-50 animate-[slideDown_0.3s_ease-out]">
          <div className="flex items-center gap-3 px-4 sm:px-5 py-3 bg-emerald-50 border border-emerald-200 rounded-xl shadow-[0px_20px_40px_-10px_rgba(15,23,42,0.15)]">
            <span className="font-mono text-xs sm:text-sm text-emerald-800 truncate">{toastMessage}</span>
            <button onClick={() => setShowToast(false)} className="ml-auto text-emerald-600 hover:text-emerald-900 text-lg leading-none">×</button>
          </div>
        </div>
      )}
      ...
      {userEmail && (
        <div className="absolute top-3 right-3 sm:top-5 sm:right-6 z-30 flex items-center gap-1">
          ...
          {/* Desktop-only inline items */}
          <button onClick={() => launchTour(userId)}
            ...
            title="Show me around" aria-label="Show me around">
          ...
          <div className="hidden sm:flex ml-1 h-9 w-9 rounded-full bg-slate-900 text-white items-center justify-center font-medium text-sm shadow-[0px_2px_8px_rgba(15,23,42,0.15)]" title={userEmail}>
          ...
          <div className="relative sm:hidden" data-app-menu>
            ...
            {menuOpen && (
              <div className="absolute right-0 top-11 w-64 bg-white border border-slate-200 rounded-xl shadow-[0_20px_40px_-10px_rgba(15,23,42,0.2)] py-1 z-50">
                ...
                <button onClick={() => { setMenuOpen(false); launchTour(userId); }} className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50">Show me around</button>
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 pt-16 sm:pt-8 pb-6 sm:pb-8">
        <div className="flex items-center gap-3 mb-6 sm:mb-8">
          ...
          <div className="min-w-0">
            <p className="font-display text-[26px] sm:text-[32px] leading-none text-slate-900">Prompt2Print</p>
            <p className="font-mono text-[10px] sm:text-[11px] tracking-wider text-slate-500 uppercase mt-1">built by a teacher, for teachers</p>
          </div>
        </div>
        <h1 className="font-display text-[36px] sm:text-[52px] lg:text-[60px] leading-[0.95] tracking-tight text-slate-900">
          Get your Sundays<br /><span className="italic text-slate-600">back.</span>
        </h1>
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 pb-10 sm:pb-12">
        <div className={`relative rounded-2xl border bg-white transition-all duration-300 ${focused ? "border-slate-400 shadow-[0_0px_0px_1px_rgba(15,23,42,0.06),_0px_40px_80px_-20px_rgba(15,23,42,0.15)]" : "border-slate-300 shadow-[0_1px_0px_0px_rgba(0,0,0,0.03),_0px_30px_70px_-15px_rgba(15,23,42,0.15)]"}`}>
          <div className="flex items-center justify-between px-5 sm:px-8 pt-5 sm:pt-7 pb-3">
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 rounded-full bg-emerald-500" />
              <label className="font-mono text-[11px] tracking-wider text-slate-600 uppercase">Your prompt</label>
            </div>
          ...
          <div data-tour="templates" className="px-5 sm:px-8 mt-5">
            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 mb-2">
              <span className="font-mono text-[11px] tracking-wider text-slate-500 uppercase">template</span>
              <span className="font-mono text-xs text-slate-400">{activeTemplate.hint}</span>
            </div>
          ...
          <div className="px-5 sm:px-8 mt-5">
            ...
            {referenceFile ? (
              <div className="flex items-center gap-3 px-3 py-2 rounded-lg border border-slate-200 bg-slate-50 w-fit max-w-full">
                ...
                <span className="font-mono text-xs text-slate-700 truncate">reference: {referenceFile.name}</span>
                <button onClick={() => { setReferenceFile(null); if (fileInputRef.current) fileInputRef.current.value = ""; }}
                  className="text-slate-400 hover:text-slate-900 text-xs flex-shrink-0">✕</button>
              </div>
            ) : (
          ...
          <div className="px-5 sm:px-8 mt-5 flex flex-wrap items-center gap-x-6 gap-y-3">
            <div data-tour="style" className="flex items-center gap-3">
              <span className="font-mono text-[11px] tracking-wider text-slate-500 uppercase">style</span>
              <div className="inline-flex p-0.5 bg-slate-100 rounded-lg">
                <button type="button" onClick={() => setStyle("rich")}
                  className={`px-3 sm:px-3.5 py-1.5 rounded-md text-xs sm:text-sm font-medium transition ${
                    style === "rich" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-900"
                  }`}>Colorful</button>
                <button type="button" onClick={() => setStyle("plain")}
                  className={`px-3 sm:px-3.5 py-1.5 rounded-md text-xs sm:text-sm font-medium transition ${
                    style === "plain" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-900"
                  }`}>Classic</button>
              </div>
            </div>
            <label data-tour="answer-key" className="inline-flex items-center gap-2 cursor-pointer select-none">
          ...
          <div className="flex flex-col gap-4 px-5 sm:px-8 py-4 sm:py-5">
            <Button onClick={generateWorksheet} size="lg"
        ...
        <footer className="mt-10 sm:mt-12 pb-8 border-t border-slate-200 pt-6 flex flex-wrap items-center justify-center gap-x-4 sm:gap-x-6 gap-y-2 font-mono text-xs text-slate-400">
          ...
          <button onClick={() => router.push("/terms")} className="hover:text-slate-700 transition underline underline-offset-2">Terms</button>
          <button onClick={() => router.push("/privacy")} className="hover:text-slate-700 transition underline underline-offset-2">Privacy</button>
          <a href="mailto:kshitijmisc@gmail.com" className="hover:text-slate-700 transition underline underline-offset-2">Contact</a>
        </footer>
      </div>
      ...
      <style jsx global>{`
        ...
        .p2p-shepherd.shepherd-element {
          border-radius: 14px; border: 1px solid rgb(226 232 240); background: white;
          box-shadow: 0 30px 80px -20px rgba(15, 23, 42, 0.25);
          max-width: min(400px, calc(100vw - 32px)); font-family: inherit;
        }
        .p2p-shepherd .shepherd-header { background: white; border-radius: 14px 14px 0 0; padding: 18px 22px 6px; }
        .p2p-shepherd .shepherd-title { font-family: 'Instrument Serif', Georgia, serif; font-size: 22px; color: rgb(15 23 42); font-weight: 400; line-height: 1.2; }
        .p2p-shepherd .shepherd-text { padding: 6px 22px 18px; color: rgb(51 65 85); font-size: 14px; line-height: 1.6; }
        .p2p-shepherd .shepherd-text p { margin: 0; }
        .p2p-shepherd .shepherd-footer { padding: 12px 18px 14px; border-top: 1px solid rgb(241 245 249); display: flex; justify-content: flex-end; gap: 8px; }
        .p2p-shepherd .shepherd-button { padding: 8px 14px; border-radius: 8px; font-size: 13px; font-weight: 500; transition: background 0.15s, color 0.15s; border: none; cursor: pointer; margin: 0; }
        .p2p-shepherd-btn-primary { background: rgb(15 23 42); color: white; }
        .p2p-shepherd-btn-primary:hover { background: rgb(30 41 59); }
        .p2p-shepherd-btn-secondary { background: transparent; color: rgb(100 116 139); }
        .p2p-shepherd-btn-secondary:hover { color: rgb(15 23 42); background: rgb(241 245 249); }
        .p2p-shepherd .shepherd-cancel-icon { color: rgb(148 163 184); font-size: 20px; }
        .p2p-shepherd .shepherd-cancel-icon:hover { color: rgb(15 23 42); }
        .p2p-shepherd .shepherd-arrow:before { background: white; border: 1px solid rgb(226 232 240); }
        .shepherd-modal-overlay-container { fill: rgba(15, 23, 42, 0.55); }
      `}</style>
    </main>
  );
}
...
export default function Home() {
  return <Suspense fallback={<div className="min-h-screen bg-[color:#FAFAF6]" />}><HomeInner /></Suspense>;
}