...
# uvicorn swallows import errors as "Attribute 'app' not found" — this makes
# the real error visible in Railway logs.
import sys
import traceback
...
def compile_endpoint(
    req: CompileRequest,
    user_id: str = Depends(get_current_user_id),  # auth-gated, but free
):
...
def generate_endpoint(
    ...
    # Deduct BEFORE generation. Atomic in Postgres — no race conditions.
    # If pipeline fails below, we refund.
    require_and_deduct_credit(user_id, "generation", {"prompt": req.prompt[:200]})
    ...
    try:
        ...
        # Total pipeline failure — refund the credit
        refund_credit(user_id, "refund_generation_failed", {"prompt": req.prompt[:200]})
        return Response(
        ...
        # Unexpected error — refund and re-raise
        refund_credit(user_id, "refund_generation_error", {"prompt": req.prompt[:200]})
        raise
...
def edit_endpoint(
    req: EditRequest,
    user_id: str = Depends(get_current_user_id),  # auth-gated, but FREE per pricing
):
    """Apply a conversational edit to an existing worksheet and return a new PDF.

    Edits are free (do not consume credits). Users can iterate freely on
    their generated worksheets — this is core to the product's magic.
...
async def generate_from_reference_endpoint(
    ...
    # Deduct BEFORE generation — refund on failure
    require_and_deduct_credit(user_id, "generation_from_reference", {"prompt": prompt[:200]})
    ...
    try:
        ...
        # Total pipeline failure — refund the credit
        refund_credit(user_id, "refund_generation_failed", {"prompt": prompt[:200]})
        return Response(
        ...
        refund_credit(user_id, "refund_generation_error", {"prompt": prompt[:200]})
        raise